...
def Download_button_clicked(root):
    ...
    if is_youtube_url(user_home_entry):
        try:
            toast.show_toast("Music Loading..","Tunggu beberapa saat, saat music sedang didowload", duration=3,icon_path=relative_to_assets("Rifqan.ico"))
        ...
        root.after(100, lambda: toast.show_toast(" Enter Something Noob ","i won't get Dreams of what you want to download!",duration=3,icon_path=relative_to_assets("Rifqan.ico")))
            
    else:
        video_uri=get_youtube_uri(user_home_entry)
        toast.show_toast("Music Didowload","Tunggu Sebentar, saat musik sedang didowload", duration=3,icon_path=relative_to_assets("Rifqan.ico"))
...
def Home(parent):
    ...
    canvas.create_text(
        ...
        58.0,
        anchor="nw",
        text="Dowload Sebuah Lagu Sekarang !!",
        fill="#C67FFC",
    ...
    canvas.create_text(
        ...
        100.0,
        anchor="nw",
        text="Dan Nikmati Memutarnya Dari List Daftar Putar kami",
        fill="#C67FFC",
